...
static string Day7()
{
    ...
    for (int i = 0; i < input.Length; i++)
    {
        ...
        if (input[i].StartsWith("$ cd "))
        {
            ...
            switch (output)
            {
                ...
                case "..":
                    ...
                    continue;
                default:
                    if (currentFolder.subFolders.Exists(x => x.name == output))
                        currentFolder = currentFolder.subFolders.First(x => x.name == output);
                    else
                    {
                        var newFolder = new Folder($"{output}", currentFolder);
        ...
        else
        {
            ...
            {
                currentFolder.files.Add(output[1], result); ;
            }
        }
    }
...
class Folder
{
    ...
    public long GetTotalFileSize()
    {
        ...
        foreach (var folder in subFolders)
            fileSize += folder.GetTotalFileSize();
        foreach (var file in files)
            fileSize += file.Value;
        return fileSize;
    }

    public long GetTotalBelow(long max)
    {
        long value = 0;
        long folderSize = 0;

        foreach (var folder in subFolders)
            value += folder.GetTotalBelow(max);

        foreach (var folder in subFolders)
            folderSize += folder.GetTotalFileSize();

        foreach (var file in files)
            folderSize += file.Value;
